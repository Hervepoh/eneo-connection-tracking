...
class Api extends BaseController
{
        ...
        if (!is_null($this->session_id)) {
            ...
            try {
                $post_fields = [
                    ...
                    {"name":"state_eneo_document_br_c","value":"' . $region_administrative . '"},
                ...
                //var_dump('post_fields',$post_fields);


                return $this->callCRM($post_fields);
            } catch (\Exception $e) {
                log_message('info', "CRM API Fail to set_entry API 'module_name:Cases' execption".$e->getMessage());
                exit($e->getMessage());
            }
        }
        log_message('info', "Fail to login in CRM API");
    }
    ...
    public function crm_update_case(array $connection)
    {
        ...
        public function crm_update_case_test_url(array $connection)
    {
        $this->session_id = $this->session_id ?? $this->get_session();
        if (!is_null($this->session_id)) {
            extract($connection, EXTR_PREFIX_SAME, "connection");
    ...
    public function crm_get_branch_info($id)
    {
       $this->session_id = $this->session_id ?? $this->get_session();
       $rest_data = [
               "session"      => $this->session_id,
               "module_name"  => "E_Bra_Branchements",
               "query"        => "e_bra_branchements_cases_c.e_bra_branchements_casescases_ida=$id",
    ...
    */
    ...
    public function generateSessionID(): ?string
    {
        ...
        $response = curl_exec($curl);
        //var_dump('post_fields',$post_fields,'curl_options',$curl_options,'response',$response);
	//die();
    ...
    protected function callCRM($post_fields, bool $all = false)
    {
	    ...
	    //$this->log('info',json_decode($post_fields['rest_data']), $response);
            $result = null;
        } else {
